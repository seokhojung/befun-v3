...
/**
 * Draw top view of the desk
 * Shows: width (horizontal) x depth (vertical)
 */
export function drawTopView(doc: typeof PDFDocument, design: DrawingDesignData, startY: number): number {
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .text('평면도 (Top View)', MARGIN, startY, { align: 'left' })

  // Draw rectangle (desk top view)
  doc
    .rect(rectX, rectY, widthPt, depthPt)
    .stroke()

  // Draw horizontal dimension (width)
  drawHorizontalDimension(doc, rectX, rectY - 30, widthPt, widthMm)

  // Draw vertical dimension (depth)
  drawVerticalDimension(doc, rectX + widthPt + 30, rectY, depthPt, depthMm)

  return rectY + depthPt + 50
}
